...
def extract_and_sort_data(data, selected_video):
    ...
    if extracted_data:
        ...
        for item in extracted_data:
            ...
            item['label_std_dev'] = label_time_summary[label]['std_dev']  # 標準偏差を追加
...
def plot_mean_with_error_bars(data, excel_name):
    ...
    for i, bar in enumerate(bars):
        yval = bar.get_height()
        ax.text(bar.get_x() + bar.get_width()/2, yval, f'{means[i]:.2f}', ha='center', va='bottom', fontsize=10, color='black')
        ax.text(bar.get_x() + bar.get_width()/2, yval - std_devs[i], f'±{std_devs[i]:.2f}', ha='center', va='top', fontsize=10, color='black')

    plt.savefig(f"{excel_name}_mean_with_error_bars.png")  # グラフを保存
    plt.show()
